...
class etl(object):
    ...
    ### NYT loading tools
    def load_nyt(fn="../data/covid-19-data/us-counties.csv"):
        df = pd.read_csv(fn)
        df = df[df['cases'] > 0]
        df = df[df['fips'] > 0]
        df['fips'].fillna(0, inplace=True)
        df = df.apply(self.clean_nyt, axis=1)
        return df
    ...
    ### Counties loading tools
    def load_counties(fn="./data/County Physical.csv"):
        df = pd.read_csv(fn, sep='\t')
        df = df.apply(self.clean_counties, axis=1)
    ...
    ### Public transport loading tools
    def load_public_transport(fn="./data/Census/lots_of_census_data.csv"):
        df = pd.read_csv(fn)
        df = df.apply(self.get_fips, axis=1)
        ret = df[[
            'fips',
        ...
        ret.acolumns = ['fips', 'percent_commuter']
        ret = ret.apply(self.clean_pubtrans, axis=1)
        return ret

    def clean_pubtrans(row):
    ...
    def load_edu(fn="./data/Census/edu.csv"):
        df = pd.read_csv(fn)
        df = df.apply(self.get_fips, axis=1)
        ret = df[[
            'fips',
    ...
    def load_housing(fn="./data/Census/housing.csv"):
        df = pd.read_csv(fn)
        ...
        ret = ret.apply(self.clean_housing, axis=1)
        return ret
    
    def clean_housing(row):
    ...
    def load_election(fn="./data/2016_US_County_Level_Presidential_Results.csv"):
        df = pd.read_csv(fn)
        ret = df[[
            'combined_fips',
            'per_dem',
